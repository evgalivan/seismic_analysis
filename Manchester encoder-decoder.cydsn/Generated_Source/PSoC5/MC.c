...
void MC_Start(void) 
{
    ...
    MC_CLKEN |= MC_CLKEN_MASK;
	MC_CLKSTBY |= MC_CLKSTBY_MASK;
}
...
void MC_Stop(void) 
{
    ...
    MC_CLKEN &= (uint8)(~MC_CLKEN_MASK);
	MC_CLKSTBY &= (uint8)(~MC_CLKSTBY_MASK);
}
...
void MC_StopBlock(void) 
{
    if ((MC_CLKEN & MC_CLKEN_MASK) != 0u)
    {
...
#if defined(MC__CFG3)
        CLK_DIST_AMASK = MC_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = MC_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(MC_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(MC_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: MC_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = MC_GetSourceRegister();
    uint16 oldDivider = MC_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = MC_CLKEN & MC_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((MC_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: MC_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(MC_DIV_PTR);
}


/*******************************************************************************
* Function Name: MC_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    MC_MOD_SRC |= modeBitMask & (uint8)MC_MODE_MASK;
}


/*******************************************************************************
* Function Name: MC_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    MC_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(MC_MODE_MASK));
}


/*******************************************************************************
* Function Name: MC_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return MC_MOD_SRC & (uint8)(MC_MODE_MASK);
}


/*******************************************************************************
* Function Name: MC_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = MC_GetDividerRegister();
    uint8 oldSrc = MC_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        MC_MOD_SRC |= CYCLK_SSS;
        MC_MOD_SRC =
            (MC_MOD_SRC & (uint8)(~MC_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: MC_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return MC_MOD_SRC & MC_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: MC_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    MC_PHASE = clkPhase & MC_PHASE_MASK;
}


/*******************************************************************************
* Function Name: MC_GetPhase
********************************************************************************
*
* Summary:
