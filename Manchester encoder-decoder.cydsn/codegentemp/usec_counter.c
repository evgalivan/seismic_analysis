                                   ...
                                   
{
        ...
        usec_counter_GLOBAL_ENABLE &= ((uint8)(~usec_counter_BLOCK_EN_MASK));
    #else
        CY_SET_REG16(usec_counter_COUNTER_LSB_PTR, counter);
    #endif /* (usec_counter_UsingFixedFunction) */
}
#endif /* (!(usec_counter_UsingFixedFunction && (CY_PSOC5A))) */
...
uint16 usec_counter_ReadCounter(void) 
{
    ...
    #if(usec_counter_UsingFixedFunction)
        return ((uint16)CY_GET_REG16(usec_counter_STATICCOUNT_LSB_PTR));
    #else
        return (CY_GET_REG16(usec_counter_STATICCOUNT_LSB_PTR));
    #endif /* (usec_counter_UsingFixedFunction) */
}
...
void usec_counter_WritePeriod(uint16 period) 
{
    ...
    #else
        CY_SET_REG16(usec_counter_PERIOD_LSB_PTR, period);
    #endif /* (usec_counter_UsingFixedFunction) */
}
...
{
    #if(usec_counter_UsingFixedFunction)
        return ((uint16)CY_GET_REG16(usec_counter_PERIOD_LSB_PTR));
    #else
        return (CY_GET_REG16(usec_counter_PERIOD_LSB_PTR));
    #endif /* (usec_counter_UsingFixedFunction) */
}
                                   ...
                                   
{
    ...
    #else
        CY_SET_REG16(usec_counter_COMPARE_LSB_PTR, compare);
    #endif /* (usec_counter_UsingFixedFunction) */
}
...
{
    return (CY_GET_REG16(usec_counter_COMPARE_LSB_PTR));
}
