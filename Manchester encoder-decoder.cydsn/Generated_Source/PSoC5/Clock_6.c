...
void Clock_6_Start(void) 
{
    ...
    Clock_6_CLKEN |= Clock_6_CLKEN_MASK;
	Clock_6_CLKSTBY |= Clock_6_CLKSTBY_MASK;
}
...
void Clock_6_Stop(void) 
{
    ...
    Clock_6_CLKEN &= (uint8)(~Clock_6_CLKEN_MASK);
	Clock_6_CLKSTBY &= (uint8)(~Clock_6_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_6_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_StopBlock(void) 
{
    if ((Clock_6_CLKEN & Clock_6_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_6__CFG3)
        CLK_DIST_AMASK = Clock_6_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_6_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_6_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_6_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_6_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_6_CLKSTBY &= (uint8)(~Clock_6_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_6_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_6_GetSourceRegister();
    uint16 oldDivider = Clock_6_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_6_CLKEN & Clock_6_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_6_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clock_6_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_6_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_SetModeRegister(uint8 modeBitMask) 
{
    Clock_6_MOD_SRC |= modeBitMask & (uint8)Clock_6_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_6_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_6_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_6_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_6_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_6_GetModeRegister(void) 
{
    return Clock_6_MOD_SRC & (uint8)(Clock_6_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_6_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_6_GetDividerRegister();
    uint8 oldSrc = Clock_6_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_6_MOD_SRC |= CYCLK_SSS;
        Clock_6_MOD_SRC =
            (Clock_6_MOD_SRC & (uint8)(~Clock_6_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_6_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_6_GetSourceRegister(void) 
{
    return Clock_6_MOD_SRC & Clock_6_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_6_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_6_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_6_PHASE = clkPhase & Clock_6_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_6_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_6_GetPhaseRegister(void) 
{
    return Clock_6_PHASE & Clock_6_PHASE_MASK;
}
